...
// Converted from terminal.js bookmarklet

// Listen for export trigger from popup
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'startExport') {
    console.log('🚀 Export triggered from extension popup');
...
function setupEnhancedClaudeExporter() {
  ...
  async function extractClaudeResponses() {
    console.log('🤖 Extracting Claude responses directly from DOM...');

    // Find all Claude response containers
    // Claude's responses are typically in divs that are siblings to user messages
    const allMessageContainers = document.querySelectorAll('[data-testid="user-message"], [data-testid="assistant-message"], .font-claude-message');

    // Alternative: Find by structure
    const conversationContainer = document.querySelector('main') || document.body;
    const allParagraphs = conversationContainer.querySelectorAll('p, pre, code');

    // Try method 1: Look for message containers
    const messageBlocks = conversationContainer.querySelectorAll('div[class*="group"]');
    ...
    for (let block of messageBlocks) {
      ...
      // Try to extract text content from Claude's response
      const textContent = block.textContent || block.innerText;

      // Only capture substantial responses
      if (textContent && textContent.length > 50) {
        // Check if not already captured
        if (!capturedResponses.some(r => r.content === textContent)) {
          ...
          console.log(`  ✓ Captured Claude response ${responseIndex} (${textContent.length} chars)`);
        }
      }
    }

    // Method 2: If method 1 didn't work, try finding response by looking for copy buttons
    if (capturedResponses.length === 0) {
      console.log('  🔄 Trying alternative extraction method...');
      const copyButtons = document.querySelectorAll(SELECTORS.copyButton);

      for (let button of copyButtons) {
        // Find the parent container with the response content
        let container = button.closest('div[class*="group"]') || button.closest('article') || button.closest('section');

        if (container) {
          // Get all text content but exclude buttons and UI elements
          ...
          if (content && content.length > 50 && !capturedResponses.some(r => r.content === content)) {
            capturedResponses.push({
              type: 'claude',
              content: content.trim(),
              timestamp: Date.now(),
              index: capturedResponses.length
            });
            console.log(`  ✓ Captured response via copy button ${capturedResponses.length} (${content.length} chars)`);
          }
        }
      }
    }

    console.log(`✅ Extracted ${capturedResponses.length} Claude responses`);
    updateStatus();
  }

  // ============= CLIPBOARD INTERCEPTION (Fallback) =============

  navigator.clipboard.writeText = function(text) {
    if (interceptorActive && text && text.length > 20) {
      console.log(`📋 Captured Claude response ${capturedResponses.length + 1} via clipboard`);
      if (!capturedResponses.some(r => r.content === text)) {
        capturedResponses.push({
          type: 'claude',
          content: text,
          timestamp: Date.now()
  ...
  // Modified download function to use Chrome extension API
  function downloadMarkdown(content, filename) {
    ...
    chrome.runtime.sendMessage({
      ...
      if (chrome.runtime.lastError) {
        ...
        statusDiv.textContent = `❌ Download failed: ${response.error}`;
        statusDiv.style.background = '#f44336';
      } else if (response && response.success) {
        ...
        console.log('✅ Download request sent (no response received)');
      }
    });
  }
  ...
  async function startExport() {
    try {
      ...
      // Step 4: Extract Claude responses directly from DOM
      statusDiv.textContent = '🤖 Extracting Claude responses...';
      await extractClaudeResponses();
      await delay(DELAYS.artifact);
  ...
  // Initialize
  updateStatus();
  // Note: Removed auto-execution. Export is now triggered via extension popup
